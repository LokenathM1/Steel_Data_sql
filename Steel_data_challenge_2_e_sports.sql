...
select t.team_name , count(p.player_id) as Teams_Strenght
from players as p 
join teams as t using(team_id)
group by t.team_id ;
...
select t.team_name as Teams , count(m.winner_id) as Matches_Won
from matches as m 
join teams as t 
...
Select round(avg(p.salary)) as Average_Salary_of_USA_Players
from players as p
join teams as t using(team_id)
where t.country = "USA";
...
select t.team_name , count(p.salary)  as `Player_with_100k+_salary`
from players as p
join teams as t using(team_id)
where p.salary > 100000
group by t.team_name
order by count(p.salary) desc ;
...
/*Bonus Question yearly salary payout to players by teams*/

select t.team_name as Teams , sum(p.salary) as Yearly_Payout
from players as p 
join teams as t using(team_id)
group by p.team_id
